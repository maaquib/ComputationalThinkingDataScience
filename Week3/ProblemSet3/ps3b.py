...
def simulationWithoutDrug(numViruses, maxPop, maxBirthProb, clearProb,
    ...
    for trial in range(numTrials):
        ...
        for i in range(300):
            populationSizeSum[i] += patient.update()
    for k, v in sorted(populationSizeSum.items()):
        trials.append(float(v) / float(numTrials))
    pylab.title("SimpleVirus simulation")
    pylab.xlabel("Time Steps")
    pylab.ylabel("Average Virus Population")
    pylab.legend()
    pylab.plot(range(300), trials)
    pylab.show()
...
class ResistantVirus(SimpleVirus):
    ...
    def reproduce(self, popDensity, activeDrugs):
        ...
        NoChildException if this virus particle does not reproduce.
        """
        if (all(self.isResistantTo(drug) for drug in activeDrugs) and random.random() <= self.getMaxBirthProb() * (1 - popDensity)):
...
def simulationWithDrug(numViruses, maxPop, maxBirthProb, clearProb, resistances,
    ...
    """
    population1 = [[] for i in range(300)]
    population2 = [[] for i in range(300)]
    for trial in range(numTrials):
        ...
        for i in range(300):
            ...
            patient.update()
            population1[i].append(patient.getTotalPop())
            population2[i].append(patient.getResistPop(["guttagonol"]))
            
    trials1 = [float(sum(list)) / float(len(list)) for list in population1]
    trials2 = [float(sum(list)) / float(len(list)) for list in population2]
