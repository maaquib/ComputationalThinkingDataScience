...
class RectangularRoom(object):
    ...
    def cleanTileAtPosition(self, pos):
        ...
        pos: a Position
        """
        if not self.clean[int(math.floor(pos.getX()))][int(math.floor(pos.getY()))]:
            self.cleaned += 1
            self.clean[int(math.floor(pos.getX()))][int(math.floor(pos.getY()))] = True
    ...
    def isTileCleaned(self, m, n):
        ...
        """
        return self.clean[int(math.floor(m))][int(math.floor(n))]
...
class StandardRobot(Robot):
    ...
    def updatePositionAndClean(self):
        ...
        if not self.room.isPositionInRoom(newPosition):
            newPosition = self.room.getRandomPosition()
            self.direction = random.choice(range(360))
...
def runSimulation(num_robots, speed, width, height, min_coverage, num_trials,
    ...
    return float(sum(step_list)) / float(len(step_list))
