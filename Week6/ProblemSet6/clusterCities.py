...
class Cluster(object):
    ...
    def singleLinkageDist(self, other):
        ...
        for point1 in self.points:
            for point2 in other.points:
                if point1.distance(point2) < shortest:
    ...
    def maxLinkageDist(self, other):
        ...
        for point1 in self.points:
            for point2 in other.points:
                if point1.distance(point2) > longest:
                    longest = point1.distance(point2)
        return longest
    ...
    def averageLinkageDist(self, other):
        ...
        return float(total) / float(len(self.points) + len(other.points))
...
class ClusterSet(object):
    ...
    def mergeClusters(self, c1, c2):
        ...
        newCluster = Cluster(set(c1.points).union(set(c2.points)), c1.pointType)
    ...
    def findClosest(self, linkage):
        ...
        for cluster1 in self.getClusters():
            for cluster2 in self.getClusters():
                if cluster1 != cluster2 and linkage(cluster1, cluster2) < dist:
                    c1, c2, dist = cluster1, cluster2, linkage(cluster1, cluster2)
        return c1, c2
...
def test():
    points = buildCityPoints('C:\Users\Aaqib\Downloads\ComputationalThinkingDataScience\Week6\ProblemSet6\cityTemps.txt', False)
